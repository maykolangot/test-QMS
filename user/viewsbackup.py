...
def get_next_transaction(user):
    today = localdate()  # Ensures timezone-aware date (e.g., Asia/Manila)
...
def next_queue(request):
    ...
    print(f"[DEBUG] Resolving next_queue for user: {user.name}")

    today = localdate()

    # Step 1: Complete last NF1 transaction if exists (today only)
    ...
    if current_txn_nf1:
        print(f"[DEBUG] Completing IN_PROCESS transaction: {current_txn_nf1.queueNumber}")
        ...
        print(f"[DEBUG] Verified NF1 {verified_txn.queueNumber} updated to: {verified_txn.status}")

        # Directly complete the legacy transaction using the same pk
        ...
        if legacy_txn:
            ...
            print(f"[DEBUG] Legacy transaction {legacy_txn.queueNumber} updated to COMPLETED")
        else:
            print(f"[DEBUG] No legacy transaction found for PK={current_txn_nf1.pk}")

    # Step 2: Ensure user has no other active IN_PROCESS transaction
    ...
    # Step 3: Safely get and reserve the next transaction
    next_txn = get_next_transaction(user)

    if next_txn:
        print(f"[DEBUG] Reserving next transaction: {next_txn.queueNumber}")
        ...
        if legacy_txn:
            ...
            print(f"[DEBUG] Legacy transaction {legacy_txn.queueNumber} updated to IN_PROCESS")
        else:
            print(f"[DEBUG] No legacy found for transaction PK={next_txn.pk}")
...
def skip_queue(request):
    ...
    # Step 1: Find the current legacy transaction
    txn = Transaction.objects.filter(
        reservedBy=user,
        status=Transaction.Status.IN_PROCESS
    ).order_by('-created_at').first()
    ...
    if txn:
        # Step 2: Cancel legacy transaction
        txn.status = Transaction.Status.CANCELLED
        txn.save()

        # Step 3: Mirror update to NF1 (bulk-safe)
        updated_count = TransactionNF1.objects.filter(queueNumber=txn.queueNumber).update(
            status=TransactionNF1.Status.CANCELLED
        )

        if updated_count == 0:
            print(f"[DEBUG] NF1 transaction not found for queueNumber {txn.queueNumber}")
    else:
        print("[DEBUG] No legacy transaction found in-process for current user.")
...
def hold_queue(request):
    ...
    if txn:
        ...
        # ✅ Mirror update in new TransactionNF1 (bulk-safe)
        updated_count = TransactionNF1.objects.filter(queueNumber=txn.queueNumber).update(
        ...
        if updated_count == 0:
            print(f"[DEBUG] No matching NF1 transaction for queue {txn.queueNumber}")
...
def update_hold_status(request):
    ...
    try:
        ...
        # ✅ Mirror update in TransactionNF1 using queueNumber (bulk-safe)
        updated_count = TransactionNF1.objects.filter(queueNumber=txn.queueNumber).update(status=new_status)
        if updated_count == 0:
            print(f"[DEBUG] No matching NF1 transaction for queueNumber {txn.queueNumber}")
        ...
        print(f"[ERROR] Unexpected error during update_hold_status: {e}")
...
def admin_queue_settings(request):
    ...
    message = None
    tz = get_current_timezone()  # Will resolve to Asia/Manila

    if request.method == "POST":
        campus = request.POST.get('campus') or None
        cutoff_time_input = request.POST.get('cutoff_time')

        # Parse and assume input is in Asia/Manila
        ...
        print(f"[DEBUG] Creating CutoffSchedule — campus={campus}, cutoff_time={cutoff_time_utc}, now={now_utc}, is_cutoff={is_cutoff_now}")
        ...
        print(f"[DEBUG] Created CutoffSchedule ID {cutoff.id} → is_cutoff={cutoff.is_cutoff}")
        ...
        if is_cutoff_now:
            ...
            print(f"[DEBUG] Applied immediate cutoff — NF1: {updated_nf1}, Legacy: {updated_legacy}")
    ...
    # Get today's cutoffs using Asia/Manila time boundaries
...
def kpi_data(request):
    ...
    # Fetch grouped counts by status and date
    raw_counts = (
        TransactionNF1.objects
        .filter(
    ...
    # Structure data per day per status
    ...
    # Forecast using linear regression per status
    forecast = {}
    for status in tracked_statuses:
...
def status_by_department_chart_data(request):
    ...
    for status in status_keys:
        datasets.append({
            ...
            # "stack": "a" removed to enable grouped bar chart
        })
...
@require_GET
def transaction_treemap_chart_data(request):
    ...
    if department_filter:
        queryset = queryset.filter(course__department__id=department_filter)

    data_map = defaultdict(lambda: defaultdict(int))  # department -> transactionType -> count
    total = 0

    for txn in queryset.select_related("course__department"):
        department = txn.course.department.name.strip().title() if txn.course and txn.course.department else "Unknown Department"
        ttype = txn.transactionType.strip().title() if txn.transactionType else "Unknown"
        data_map[department][ttype] += 1
        total += 1

    if total == 0:
        return JsonResponse({"data": []})

    # Build flat data for JS
    flat_items = [("Araullo University", total, "")]  # root node
    for dept, types in data_map.items():
        dept_total = sum(types.values())
        flat_items.append((dept, dept_total, "Araullo University"))
        for ttype, count in types.items():
            flat_items.append((ttype, count, dept))

    color_palette = ["#4dc9f6", "#f67019", "#f53794", "#537bc4", "#acc236", "#00a950", "#9966FF", "#FF9F40"]
    data = []
    for i, (label, value, parent) in enumerate(flat_items):
...
def heatmap_chart_data(request):
    ...
    # Filter categories (X-axis): group by transactionType
    # Rows (Y-axis): date labels
...
def hourly_heatmap_chart_data(request):
    ...
    for txn in queryset:
        hour_label = f"{txn.created_at.hour:02}:00"
        date_label = str(txn.created_at.date())
...
def forecast_chart_data(request):
    ...
    for txn in queryset:
        ...
        hour_totals[hour] += 1
        day_set.add(txn.created_at.date())

    total_days = len(day_set) or 1
    avg_by_hour = {h: hour_totals[h] / total_days for h in range(24)}

    # Step 4: Prepare labels and values
    labels = [str(d) for d in date_range]
    values = [daily_counts.get(d, 0) for d in date_range]
...
def priority_breakdown_view(request):
    ...
    # Queryset: completed transactions only
    queryset = TransactionNF1.objects.filter(
        status__iexact="completed",
        created_at__date__gte=start_date
    )
    ...
    # Safely handle priority values
    priority_list = []
    for txn in queryset:
        priority_list.append(bool(txn.priority))
